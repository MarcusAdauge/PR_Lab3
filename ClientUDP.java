...
public class ClientUDP extends Application { 
	...
	public void start(Stage primaryStage) throws Exception {
		...
		window.setOnCloseRequest(e -> {
			...
			try {
				if(allUsers.size() > 1) socket.cast(GOODBYE, me);
		...
		btnClear.setOnAction(e -> {
			if(allUsers.size() > 1) socket.cast(CLEAR_CANVAS, me);
			else {
	...
	public void Painting(){
		canvas.setOnMousePressed((MouseEvent e) -> {
			...
			ClientUDP.gc.moveTo(e.getX(), e.getY());
			//ClientUDP.gc.stroke();
			socket.cast(ClientUDP.START_DRAW, new point2d(e.getX(), e.getY(), ClientUDP.me).setStroke(current_stroke_width));
        });
		
		canvas.setOnMouseDragged((MouseEvent e) -> {
			ClientUDP.gc.lineTo(e.getX(), e.getY());
			//ClientUDP.gc.stroke();
			socket.cast(ClientUDP.DRAW, new point2d(e.getX(), e.getY(), ClientUDP.me));
        });
        
		canvas.setOnMouseReleased((MouseEvent e) -> socket.cast(ClientUDP.END_DRAW, null));
	...
	public static void submission(){
		socket.cast(I_AM_NEW, null);	// introduce yourself
		Object obj = null;
		int[] type = new int[1];
		...
		socket.cast(SUBMIT_ME, me);
	}
	
	private static boolean isColorSimilar(Color mine, Color notMine){
		boolean result = false;
		// it is OK if at least one of the RGB value is not in the range +-0.2
