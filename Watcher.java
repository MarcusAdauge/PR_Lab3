				...
				try{
					...
					switch(type[0]){
						...
						case ClientUDP.I_AM_NEW:
							...
							sock.send(ClientUDP.GREETINGS, ClientUDP.allUsers);
							break;
	...
	void decisionMaking(Color userColor){
		Platform.runLater(new Runnable() {
            ...
            public void run() {
               ...
               // position the decision window in the middle of the parent window
               ...
               btnYES.setOnAction(e -> {
					sock.send(ClientUDP.CLEAR_CANVAS_RESPONSE, new Boolean(true));
            	   decisionWindow.close();
               });
               
               btnNO.setOnAction(e -> {
            	   sock.send(ClientUDP.CLEAR_CANVAS_RESPONSE, new Boolean(false));
            	   decisionWindow.close();
               });
	...
	private void processDecissionsForCLEAR_CANVAS(Boolean resp){
			...
			if(decission > (ClientUDP.allUsers.size() / 2)){ 
				...
				Platform.runLater(new Runnable() {
	                @Override
	                public void run() {
	                   ClientUDP.logStack.getChildren().add(new Label("Clear-Board failed!"));
	                }
	            });
			}
